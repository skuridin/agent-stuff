...
For a guided walk-through on setting default values, see the container system config tutorial. 

Source of truth: `Sources/ContainerPersistence/ContainerSystemConfig.swift`.
...
| `memory`  | MemorySize  | `"2048mb"`                                           | RAM allocation for the builder VM. |
...
| `memory` | MemorySize | `"1g"`  | Default RAM per container. |
...
Guest kernel used when launching container VMs. Defaults change per release as kernels are bumped.
...
| Key        | Type       | Default | Description                                                                                       |
|------------|------------|---------|---------------------------------------------------------------------------------------------------|
| `subnet`   | CIDRv4?    | unset   | IPv4 CIDR (e.g. `"192.168.100.0/24"`). When unset, the system auto-allocates a non-overlapping subnet. |
| `subnetv6` | CIDRv6?    | unset   | IPv6 CIDR (e.g. `"fd00:abcd::/64"`). When unset, the system auto-allocates.                       |
